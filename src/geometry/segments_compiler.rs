...
fn compile_geometry_file(file: &AntennaFile, segment_size_divider:f64) -> Antenna {
    ...
    for wire in &file.wires {
        ...
        let first_half= segment_line(p_start, midpoint, segment_size, &mut nodes, &mut segments);
        let second_half = segment_line(midpoint, p_end, segment_size, &mut nodes, &mut segments);
        ...
        println!("Inserting wire: id = {:?}, metadata = {:?}", wire.id, wire_metadata);
    ...
    Antenna { nodes, segments, wire_map }
}
...
mod tests {
    ...
    fn test_compile_geometry_file_dipole() {
        ...
        for i in 1..11 {
            assert_eq!(antenna.nodes[i].incidence, 2, "Node {} has incidence {}, expected 2", i, antenna.nodes[i].incidence);
        }
    }
    ...
    fn test_compile_geometry_file_vertical() {
        ...
        for i in 0..23 {
            ...
            assert_eq!(antenna.nodes[i].incidence,incidence, "Node {} has incidence {}, expected 2", i, antenna.nodes[i].incidence);
        }
    }
}